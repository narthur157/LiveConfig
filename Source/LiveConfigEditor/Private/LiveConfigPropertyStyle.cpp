...
// Copied from GameplayTagStyle.cpp
class FLiveConfigPropertyStyleSet final : public FSlateStyleSet
{
public:
	FLiveConfigPropertyStyleSet(const FName& InStyleSetName)
		: FSlateStyleSet(InStyleSetName)
	{
		...
		Set("LiveConfigProperty.ChipButton.Selected", ChipButtonSelected);
		...
		Set("LiveConfigProperty.ChipButton.Unselected", ChipButtonUnselected);
		...
		Set("LiveConfigProperty.ChipClearButton", ChipClearButton);

		Set("LiveConfigProperty.Container", new FSlateRoundedBoxBrush(FStyleColors::Input, 4.0f, FStyleColors::InputOutline, 1.0f));
		...
		Set("LiveConfigProperty.ComboButton", TagComboButton);
	}
};


TSharedPtr<FSlateStyleSet> FLiveConfigPropertyStyle::StyleSet = nullptr;



void FLiveConfigPropertyStyle::Initialize()
{
	if (StyleSet.IsValid())
	{
		return;
	...
	StyleSet = MakeShared<FLiveConfigPropertyStyleSet>(GetStyleSetName());
	
	FSlateStyleRegistry::RegisterSlateStyle(*StyleSet.Get());
}

void FLiveConfigPropertyStyle::Shutdown()
{
	if (StyleSet.IsValid())
	{
...
FName FLiveConfigPropertyStyle::GetStyleSetName()
{
	static FName StyleName("LiveConfigPropertyStyle");
	return StyleName;
}
